...
export class CustomValidators {
    ...
    static validatorDate(): ValidatorFn {
        return (control: AbstractControl) => {
            if (!control.value) return null;

            if (control.value?.replace(/_/g, "").length == 10 && !isNaN(new Date(control.value).getTime())) {
                return null
            } else {
    ...
    static validatorMaxDate(dt_Maximo: string): ValidatorFn {
        return (control: AbstractControl) => {
            ...
            if (control.value?.replace(/_/g, "").length == 10) {
    ...
    static validatorConfirmarSenha(ds_Campo: string): ValidatorFn {
        return (control: AbstractControl) => {
            if (!control.value || !control.root.get(ds_Campo)) return null;
    ...
    static validatorTelefone(): ValidatorFn {
        return (control: AbstractControl) => {
            if (!control.value) return null;
            const value = control.value.replace(/\(|\)|-|_/g, "")
            return value && value.indexOf("_") == -1 && value.length == 8 && !["11111111", "22222222", "33333333", "44444444", "55555555", "66666666", "77777777", "88888888", "99999999", "00000000"].includes(value) ? null : { pattern: true };
        }
    }
